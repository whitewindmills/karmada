...
func (tc *NoExecuteTaintManager) syncCluster(ctx context.Context, cluster *clusterv1alpha1.Cluster) (reconcile.Result, error) {
	...
	for i := range rbList.Items {
		...
		tc.enqueueBinding(tc.bindingEvictionWorker, noExecuteTaints, rbList.Items[i].Annotations, key, now)
	}
	...
	for i := range crbList.Items {
		...
		tc.enqueueBinding(tc.clusterBindingEvictionWorker, noExecuteTaints, crbList.Items[i].Annotations, key, now)
	}
...
// given taints and enqueues it to the provided worker appropriately:
...
//
// The worker must match the binding kind of key (bindingEvictionWorker for
// ResourceBinding, clusterBindingEvictionWorker for ClusterResourceBinding).
func (tc *NoExecuteTaintManager) enqueueBinding(worker util.AsyncWorker, taints []corev1.Taint, annotations map[string]string, key keys.FederatedKey, now time.Time) {
	if len(taints) == 0 {
		return
	}
	...
	case minTolerationTime == 0:
		worker.Add(key)
	case minTolerationTime > 0:
		worker.AddAfter(key, minTolerationTime)
	}
...
func (tc *NoExecuteTaintManager) syncBindingEviction(key util.QueueKey) error {
	...
	var failedBinding *workv1alpha2.ResourceBinding
	...
	if failedBinding != nil {
		helper.EmitClusterEvictionEventForResourceBinding(failedBinding, cluster, tc.EventRecorder, err)
		klog.ErrorS(err, "Failed to update binding", "binding", klog.KObj(failedBinding))
	}
	return err
}
...
func (tc *NoExecuteTaintManager) syncClusterBindingEviction(key util.QueueKey) error {
	...
	var failedBinding *workv1alpha2.ClusterResourceBinding
	err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		failedBinding = nil
	...
	if failedBinding != nil {
		helper.EmitClusterEvictionEventForClusterResourceBinding(failedBinding, cluster, tc.EventRecorder, err)
		klog.ErrorS(err, "Failed to update cluster binding", "binding", failedBinding.Name)
	}
	return err
}
