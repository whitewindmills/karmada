...
func TestGetObjectPerPodMetricReplicas(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:               "Scale with fractional calibration",
			statusReplicas:     2,
			targetAverageUsage: 10,
			metricName:         "requests_per_pod",
			namespace:          defaultNamespace,
		...
		{
			name:               "Scale up with duplicated replicas",
			statusReplicas:     6,
			targetAverageUsage: 10,
			metricName:         "requests_per_pod",
			namespace:          defaultNamespace,
		...
		{
			name:               "Scale down with duplicated replicas",
			statusReplicas:     12,
			targetAverageUsage: 10,
			metricName:         "requests_per_pod",
			namespace:          defaultNamespace,
		...
		{
			name:               "No change due to tolerance with duplicated replicas",
			statusReplicas:     6,
			targetAverageUsage: 10,
			metricName:         "requests_per_pod",
			namespace:          defaultNamespace,
		...
		{
			name:               "Round up after fractional calibration",
			statusReplicas:     2,
			targetAverageUsage: 10,
			metricName:         "requests_per_pod",
			namespace:          defaultNamespace,
